...
export default function PanelPedidosLocal() {
  ...
  const sonidoRef = useRef(
    typeof Audio !== "undefined" ? new Audio("/sounds/notificacion.mp3") : null
  );

  // Normaliza y MUESTRA fecha y hora legible, preferente desde fechaCreacion
  const formatearFechaHora = (p) => {
    const raw =
      p.fechaCreacion || // principal desde backend
      p.fecha ||
      p.createdAt ||
      p.fechaHora;
    if (!raw) return null;
    ...
    if (isNaN(d)) {
      // fallback seguro si viene como "YYYY-MM-DD"
  ...
  // Para filtrar por día, extrae solo YYYY-MM-DD del campo de fecha
  const normalizarSoloFecha = (p) => {
    const raw =
      p.fechaCreacion || p.fecha || p.createdAt || p.fechaHora;
    if (!raw) return null;
    try {
      // si es ISO con zona, usa Date
      const d = new Date(raw);
      if (!isNaN(d)) {
        // YYYY-MM-DD local
  ...
  const eliminarPedido = async (idPedido, pedidoIdStr) => {
    const ok = window.confirm(
      `¿Eliminar el pedido #${idPedido}? Esta acción es irreversible.`
    );
    if (!ok) return;
    try {
      ...
      if (e?.response?.status === 404 && pedidoIdStr) {
        try {
          await axios.delete(
            `${API_BASE_URL}/api/pedidos/by-pedido-id/${encodeURIComponent(
              pedidoIdStr
            )}`
          );
          setPedidos((prev) => prev.filter((p) => p.id !== idPedido));
  ...
  const limpiarLista = () => {
    const ok = window.confirm(
      "¿Vaciar la lista visible? No elimina datos del backend."
    );
    if (!ok) return;
    setPedidos([]);
  };
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    return base.filter((p) => normalizarSoloFecha(p) === filtroFecha);
      ...
      <div className="panel">
        <div className="caja-blanca">
          ...
          >
            ...
            <button className="boton-accion" onClick={obtenerPedidos}>
              Actualizar
            </button>
            <button
              className="boton-accion boton-retroceder"
              onClick={limpiarLista}
              title="Vacía la tabla local"
            >
              Limpiar lista
            </button>
          </div>
